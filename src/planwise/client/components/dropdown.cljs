(ns planwise.client.components.dropdown
...
;;  Base implementation taken from https://github.com/Day8/re-com
;;
;;  To this we added the ability to define a separate render-fn to
;;  allow defining custom markup without compromising the components'
;;  filter capabilities
...
(def single-dropdown-args-desc
  [{:name :choices       :required true                   :type "vector of choices | atom"      :validate-fn vector-of-maps?   :description [:span "Each is expected to have an id, label and, optionally, a group, provided by " [:code ":id-fn"] ", " [:code ":label-fn"] " & " [:code ":group-fn"]]}
   ...
   {:name :render-fn     :required false                  :type "choice -> string | hiccup"     :validate-fn ifn?              :description [:span "given an element of " [:code ":choices"] ", returns the html that will be rendered. defaults to the label if no custom markup is required."]}
