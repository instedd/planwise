-- :name select-regions :? :*
SELECT id, country, name, admin_level AS "admin-level",
       ST_AsGeoJSON(ST_Envelope(the_geom)) AS "bbox",
...
SELECT id, country, name, admin_level,
       ST_AsGeoJSON(preview_geom) AS "preview-geojson",
       ST_AsGeoJSON(ST_Envelope(the_geom)) as bbox,
       total_population AS "total-population"
  FROM regions
 WHERE id IN (:v*:ids);
...
SELECT id,
       ...
       admin_level,
       ST_AsGeoJSON(preview_geom) AS "preview-geojson",
       ST_AsGeoJSON(ST_Simplify(the_geom, :simplify), 15, 3) as geojson,
       ST_AsGeoJSON(ST_Envelope(the_geom)) as bbox,
       total_population AS "total-population"
  FROM regions
 WHERE id IN (:v*:ids);
